...
import type { IdRow, UserAuthRow } from "../types/db.js";
...
if (existing.rowCount !== null && existing.rowCount > 0) {
    ...
    const countResult = await db.query("SELECT COUNT(*) as cnt FROM tipp_users");
    const cnt = parseInt(countResult.rows[0].cnt as string, 10);
    const isFirstUser = cnt === 0;
