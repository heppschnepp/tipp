...
export const GROUP_MATCHES = [
  { group: 'A', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'B', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'C', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'D', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'E', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'F', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'G', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'H', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'I', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'J', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'K', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
  { group: 'L', matches: [[0, 1], [2, 3], [0, 2], [1, 3], [0, 3], [1, 2]] },
];
...
export async function seedDatabase(): Promise<void> {
  ...
  for (const gm of GROUP_MATCHES) {
    ...
    const teamsResult = await teamsReq.query('SELECT Id, Name FROM tipp_Teams WHERE GroupName = @group ORDER BY Id');
    const teamIds = teamsResult.recordset.map((r: any) => r.Id);

    if (teamIds.length < 4) {
      console.warn(`Group ${gm.group} has only ${teamIds.length} teams, expected 4. Skipping...`);
      continue;
    }

    for (let i = 0; i < gm.matches.length; i++) {
      const [homeIdx, awayIdx] = gm.matches[i];
      const matchKey = `g${gm.group}${i}`;
  ...
  for (const kr of KNOCKOUT_ROUNDS) {
    for (let i = 0; i < kr.count; i++) {
      const roundId = 
        kr.round === 'Round of 32' ? 'r32' :
        kr.round === 'Round of 16' ? 'r16' :
        kr.round === 'Quarter-finals' ? 'qf' :
        kr.round === 'Semi-finals' ? 'sf' :
        kr.round === '3rd Place' ? '3rd' : 'f';
